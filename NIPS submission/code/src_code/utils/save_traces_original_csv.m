function [completed] = save_traces_original_csv(data,options)
...
% data=testing.data;
n=options.no_traces;
no_points=length(data.REF)/n;
% missing time
data.time=0:options.dt:options.T_train;
if ~isfield(options,'csv_filename')
    file_name='quadcopter_csv_original_';
end
    ...
    function create_csv
        ...
        for i=1:index
            ...
            for j=index_start:index_start+no_points-1
                fprintf(fid,['%.5f, %.5f,%.5f, %.5f, %.5f, %0.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %0.5f, %.5f,',...
                    '%.5f, %.5f, %.5f, %.5f, %0.5f \n'],...
                    data.time(jj),data.REF(j),data.Y(j,1),data.Y(j,2),data.Y(j,3),data.Y(j,4),data.Y(j,5),...
