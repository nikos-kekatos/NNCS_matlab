...
model.add(Dense(units=50, activation = 'tanh', input_dim = X_train.shape[1]))
...
if model_type==2:
    history=model.fit(X_train, y_train, batch_size = 10000, epochs = 100,validation_split=0.2)#,validation_data = (y_train,y_test))
elif model_type==1:
    history=model.fit(X_train, y_train, batch_size = 5000, epochs = 4000,validation_split=0.2)#,validation_data = (y_train,y_test))
...
model.save("keras_nn_akshay_minmax_norm_v2.h5")
