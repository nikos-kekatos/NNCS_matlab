% Here we specify the simulation parameters, configurations options, etc.
...
if exist('model','var')
    options.model=model;
else %we directly choose the slx model
     options.model=7;
end

% specific threshold for switching threshold
PID_TRESH=0.3;
% s=tf('s')
...
options.specs_file='specs_tutorial.stl';
% added for Breach
...
if options.reference_type==4
    ...
    options.breach_ref_min=[0 ];
    options.breach_ref_max=[1 ];
...
%     options.coverage.cells_values=options.coverage.ref_min+options.coverage.delta_resolution:options.coverage.delta_resolution:options.coverage.ref_max-options.coverage.delta_resolution;
    ...
    elseif options.coverage.m==1
        ...
        error('Need to generalize. The code is in an a different config. Please contact me.');
    end
