...
model=7; % 1: watertank, 2: robotarm, 3: quadcopter
...
elseif model==5
    SLX_model='QuadrotorSimulink_nk';
elseif model==6
    SLX_model='sf_slswitch_nk';
elseif model==7
    SLX_model='engine_cl_tuned';
end
...
elseif model==6
    ...
    options.ref_min=0;
    options.ref_max=2;
    options.sim_cov=[1;1.5];
    temp=(unique(data_cex.REF));
    options.sim_cov=temp(5)
    options.u_index_plot=1;
    options.y_index_plot=1;
    options.ref_index_plot=1;
elseif model==7
...
elseif model==7
    falsif.breach_ref_min=2000;
    falsif.breach_ref_max=2200;
end
