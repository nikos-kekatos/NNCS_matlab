...
%% A2. Iput: specify Simulink model

model=6; % 1: watertank, 2: robotarm, 3: quadcopter
...
% A3. Input: specify configuration parameters
...
% A4a. Run simulations -- Generate training data
...
% A5b. Data Preprocessing
display_ranges(data);

% A5c. Tailored data selection
options.trimming_steady_state=0;
options.trim_ss_time=7;
options.keepData_factor=1;% we keep one out of every 5 data in steady state
options.deleteData_factor=4; % we delete one every 3 data points
...
data1=data;
%%
%% B2. Iput: specify Simulink model
...
%% C2. Iput: specify Simulink model

model=6; % 1: watertank, 2: robotarm, 3: quadcopter

if model==6
    SLX_model='controller_comb_sim';
end
% load_system(SLX_model)
% Uncomment next line if you want to open the model
open(SLX_model)
...
data3=data;
%% Combine data from different controllers
[data]=combine_data(data1,data2,data3)
...
% training_options.neurons=[40 20];
...
file_name='hespanha_comb_NN';
construct_SLX_with_NN(options,file_name);

% %% 8b. Use different models for CEX
% [options]=create_NN_diagram(options,net);
% construct_SLX_with_NN(options,'controller_comb_NN_cex');
...
options.error_sd=0;%0.001;
if model==6
    options.sim_cov=data.REF([1, 2501]);
