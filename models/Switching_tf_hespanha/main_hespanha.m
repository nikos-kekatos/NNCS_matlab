...
rmpath(genpath([module_dir filesep 'models' filesep 'ParNMPC']));
...
options.trim_ss_time=3;
options.keepData_factor=4;% we keep one out of every 5 data in steady state
...
training_options.neurons=[30 20];
...
options.error_sd=0;%0.001;
if model==6
    aa=unique(data.REF,'stable');
    options.sim_cov=aa(1:2)
%         options.sim_cov=[1;0.4];
