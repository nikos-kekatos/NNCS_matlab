...
% need to train longer and falsify with more breach traces
...
if model==5
    SLX_model='e5_Quadrotor_3_blocks';
end
...
%% 5a. Data Selection
options.trimming=1;
options.keepData_factor=3;% we keep one out of every 5 data
...
%  [options]=create_NN_diagram(options,net);
%  construct_SLX_with_NN(options,'e5_Quadrotor_stable_nn_cex');
...
while i_f<=falsif.iterations_max && ~stop
    ...
    %%%%% choose data.ref (ignore 3rd reference as it does not pass as an
    %%%%% input to NN.
    
    data_cex.REF_all=data_cex;
    data_cex.REF=data_cex.REF(:,1:2);
